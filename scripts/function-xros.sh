...
enable_default_xros_architectures() {
  ...
  ENABLED_ARCHITECTURES[ARCH_ARM64_MAC_CATALYST]=0
  ENABLED_ARCHITECTURES[ARCH_ARM64_SIMULATOR]=1
  ENABLED_ARCHITECTURES[ARCH_ARM64E]=0
  ENABLED_ARCHITECTURES[ARCH_I386]=0
  ENABLED_ARCHITECTURES[ARCH_X86_64]=0
  ENABLED_ARCHITECTURES[ARCH_X86_64_MAC_CATALYST]=0
}

display_help() {
  COMMAND=$(echo "$0" | sed -e 's/\.\///g')

  echo -e "\n'$COMMAND' builds FFmpegKit for visionOS platform. By default only one architecture (arm64-simulator)\
is enabled without any external libraries. Options can be used to disable architectures and/or enable external libraries.\
...
When compilation ends, libraries are created under the prebuilt folder.\n"
  ...
  display_help_options "  -x, --xcframework\t\tbuild xcframework bundles instead of framework bundles" "  -l, --lts			build lts packages to support sdk 10+ devices" "      --target=xros sdk version\t\t\toverride minimum deployment target [12.1]"
...
get_app_specific_cflags() {
  ...
  libwebp | xvidcore)
    ...
    ;;
  sdl)
    APP_FLAGS="-DPIC -Wno-declaration-after-statement -Wno-unused-function -D__XROS__"
    ;;
  shine)
...
initialize_prebuilt_xros_folders() {
  if [[ -n ${FFMPEG_KIT_XCF_BUILD} ]]; then
    ...
    if [[ $(is_apple_architecture_variant_supported "${ARCH_VAR_XROSSIMULATOR}") -eq 1 ]]; then
      initialize_folder "${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_XROSSIMULATOR}")"
      initialize_folder "${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_XROSSIMULATOR}")"
    fi
...
create_universal_libraries_for_xros_default_frameworks() {
  ...
  create_ffmpeg_universal_library_xros_only "${ARCH_VAR_XROS}"

  create_ffmpeg_kit_universal_library_xros_only "${ARCH_VAR_XROS}"
...
create_universal_libraries_for_xros_xcframeworks() {
  ...
  create_ffmpeg_universal_library_xros_only "${ARCH_VAR_XROS}"
  create_ffmpeg_universal_library_xros_only "${ARCH_VAR_XROSSIMULATOR}"
  
  create_ffmpeg_kit_universal_library_xros_only "${ARCH_VAR_XROS}"
  create_ffmpeg_kit_universal_library_xros_only "${ARCH_VAR_XROSSIMULATOR}"
...
create_frameworks_for_xros_xcframeworks() {
  ...
  create_ffmpeg_framework "${ARCH_VAR_XROS}"
  create_ffmpeg_framework "${ARCH_VAR_XROSSIMULATOR}"

  create_ffmpeg_kit_framework "${ARCH_VAR_XROS}"
  create_ffmpeg_kit_framework "${ARCH_VAR_XROSSIMULATOR}"

  echo -e "INFO: Frameworks for xcframeworks built successfully\n" 1>>"${BASEDIR}"/build.log 2>&1
}

create_xros_xcframeworks() {
  export ARCHITECTURE_VARIANT_ARRAY=("${ARCH_VAR_XROS}" "${ARCH_VAR_XROSSIMULATOR}")
